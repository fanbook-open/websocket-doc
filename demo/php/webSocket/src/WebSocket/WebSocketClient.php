...
use WebSocket\Exception\ServerConnectException;
use WebSocket\Exception\HandshakeException;
use WebSocket\Exception\BadFrameException;
use WebSocket\Exception\SocketRWException;
...
class WebSocketClient
{
    ...
    protected function writeToSock($data)
    {
        ...
        $dataLen = strlen($data);
        if ($dataLen > $this->PACKET_SIZE) {
            $dataPieces = str_split($data, $this->PACKET_SIZE);
    ...
    protected function packFrame($opCode, $playLoad = '', $isMask = true, $status = 1000)
    {
        ...
        if ($isMask) {
            $maskBytes = array(mt_rand(1, 255), mt_rand(1, 255), mt_rand(1, 255), mt_rand(1, 255));
    ...
    private function fetchStrFromReadBuf($len = 1)
    {
        ...
        while ($target > $this->currentReadBufLen) {
            ...
            $read = fread($this->sock, $this->PACKET_SIZE);
            if (!$read) {
                $meta = stream_get_meta_data($this->sock);
                if ($meta['timed_out']) {// 超时重试
                    continue;
                }
            ...
            case self::OPCODE_CLOSE:
                if ($dataFrame->fin == 0) {
                    do {
                        ...
                        $dataFrame->payload .= $continueFrame->payload;
    ...
    protected function readFrame()
    {
        ...
        if ($dataLen > 0) {
            if ($isMasked) {
                ...
                $maskSet = array(ord($maskChars[0]), ord($maskChars[1]), ord($maskChars[2]), ord($maskChars[3]));
